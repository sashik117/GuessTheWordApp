...
public class PersistenceInitializer {
    ...
    private static final String DDL_SCRIPT_PATH = "db/ddl_sqlite.sql";
    private static final String DML_SCRIPT_PATH = "db/dml_sqlite.sql";
    private static final String CLEAR_SCRIPT_PATH = "db/ddl_clear_data_sqlite.sql";
    ...
    /**
     * Автоматична ініціалізація бази даних після створення біну.
     */
    @PostConstruct
    ...
    public void init(boolean isRunDml) {
        try (Connection connection = connectionPool.getConnection();
            ...
            // Виконуємо DDL по рядках
            executeSqlScript(statement, getSQL(DDL_SCRIPT_PATH));
            if (isRunDml) {
                // Виконуємо DML по рядках
                executeSqlScript(statement, getSQL(DML_SCRIPT_PATH));
            }
    ...
    public void clearData() {
        try (Connection connection = connectionPool.getConnection();
            ...
            executeSqlScript(statement, getSQL(CLEAR_SCRIPT_PATH));
    ...
    /**
     * Розбиває SQL-скрипт по символу ';' та виконує кожну команду окремо.
     * Пропускає порожні рядки.
     *
     * @param statement об'єкт Statement
     * @param sqlScript SQL-скрипт як рядок
     * @throws SQLException у разі помилки виконання SQL
     */
